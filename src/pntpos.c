 ...
 *          ssat_t *ssat     IO  satellite status              (NULL: no output)
 *          char   *msg      O   error message for error exit
 *          double *rs       O   satellite positions and velocities (x, y, z, vx, vy, vz) in ECEF (m, m/s)
 *                               Array of length 6*n, where n is the number of observations.
 *          int    *vsat     O   satellite visibility array (1 for valid satellite, 0 for invalid)
 *                               Array of length n, corresponding to each observation data.
 * return : status(1:ok,0:error)
 *-----------------------------------------------------------------------------*/
extern int my_pntpos(const obsd_t *obs, int n, const nav_t *nav,
                     const prcopt_t *opt, sol_t *sol, double *azel, ssat_t *ssat,
                     char *msg, double *rs, int *vsat)
{
    prcopt_t opt_ = *opt;

    // // 对于my_pntpos函数，仅使用广播星历
    // opt_.sateph = EPHOPT_BRDC;

    double *dts, *var, *azel_, *resp;
    int i, stat, svh[MAXOBS];
    ...
    // for (i = 0; i < n; i++)
    // {
    //     if (vsat[i] > 0) // 如果该卫星是有效的
    //     {
    //         int sat = obs[i].sat;     // 获取卫星编号
    //         double x = rs[i * 6];     // 获取卫星的 x 坐标
    //         double y = rs[i * 6 + 1]; // 获取卫星的 y 坐标
    //         double z = rs[i * 6 + 2]; // 获取卫星的 z 坐标

    //         // 打印有效卫星的编号和坐标
    //         printf("sat number: %d,  coordinate (x, y, z): (%.3f, %.3f, %.3f)\n", sat, x, y, z);
    //     }
    // }
